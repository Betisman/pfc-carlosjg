...
def election_shortcut(request, election_short_name):
  ...
  if election:
    return HttpResponseRedirect(settings.GET_SECURE_URL_HOST(request) + reverse(one_election_view, args=[election.uuid]))
  else:
    raise Http404
  ...
  if request.method == "GET":
    ...
    if election_form.is_valid():
      ...
      if helios_utils.urlencode(election_params['short_name']) == election_params['short_name']:      
        ...
        election_params['cast_url'] = settings.GET_SECURE_URL_HOST(request) + reverse(one_election_cast, args=[election_params['uuid']])
...
def test_cookie(request):
  ...
  return HttpResponseRedirect(settings.GET_SECURE_URL_HOST(request) + next_url)  

def test_cookie_2(request):
  continue_url = request.GET['continue_url']

  if not request.session.test_cookie_worked():
    return HttpResponseRedirect(settings.GET_SECURE_URL_HOST(request) + ("%s?%s" % (reverse(nocookies), urllib.urlencode({'continue_url': continue_url}))))
...
def new_trustee(request, election):
  if request.method == "GET":
    ...
    trustee.save()
    return HttpResponseRedirect(settings.GET_SECURE_URL_HOST(request) + reverse(list_trustees_view, args=[election.uuid]))
  ...
  if election:
    ...
    if trustee:
      if trustee.secret == trustee_secret:
        ...
        return HttpResponseRedirect(settings.GET_SECURE_URL_HOST(request) + reverse(trustee_home, args=[election.uuid, trustee.uuid]))
      else:
...
def trustee_send_url(request, election, trustee_uuid):
  ...
  url = settings.GET_SECURE_URL_HOST(request) + reverse(trustee_login, args=[election.short_name, trustee.email, trustee.secret])
...
def one_election_cast(request, election):
  ...
  """
  if request.method == "GET":
    return HttpResponseRedirect("%s%s" % (settings.GET_SECURE_URL_HOST(request), reverse(one_election_view, args = [election.uuid])))
  ...
  return HttpResponseRedirect("%s%s" % (settings.GET_SECURE_URL_HOST(request), reverse(one_election_cast_confirm, args=[election.uuid])))
...
def password_voter_login(request, election):
  ...
  if password_login_form.is_valid():
    try:
      ...
      return HttpResponseRedirect(settings.GET_SECURE_URL_HOST(request) + redirect_url)
  
  return HttpResponseRedirect(settings.GET_SECURE_URL_HOST(request) + return_url)
...
def one_election_cast_confirm(request, election):
  ...
  if request.method == "POST":
    ...
    if not voter:
      return HttpResponseRedirect(settings.GET_SECURE_URL_HOST(request) + reverse(one_election_cast_confirm, args=[election.uuid]))
...
def voter_delete(request, election, voter_uuid):
  ...
  return HttpResponseRedirect(settings.GET_SECURE_URL_HOST(request) + reverse(voters_list_pretty, args=[election.uuid]))
...
def one_election_compute_tally(request, election):
  ...
  if not _check_election_tally_type(election):
    return HttpResponseRedirect(settings.GET_SECURE_URL_HOST(request) + reverse(one_election_view,args=[election.election_id]))
...
def release_result(request, election):
  ...
  if request.method == "POST":
    ...
    return HttpResponseRedirect("%s" % (settings.GET_SECURE_URL_HOST(request) + reverse(one_election_view, args=[election.uuid])))
  ...
  if request.method == "POST":
    if bool(request.POST.get('confirm_p', 0)):
      ...
      if request.FILES.has_key('voters_file'):
        ...
        return HttpResponseRedirect("%s?%s" % (settings.GET_SECURE_URL_HOST(request) + reverse(voters_upload, args=[election.uuid]), urllib.urlencode({'e':'no voter file specified, try again'})))
