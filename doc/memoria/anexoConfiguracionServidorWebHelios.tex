...
El framework web Django provee su propio Servidor Web, pero para poder utilizar un canal seguro \gls{HTTPS} y configurar el acceso con el \gls{DNIe} es preciso utilizar un Servidor Web como Apache en el que apoyar el framework. Aquí una lista de pasos que se han llevado a cabo para su configuración. 
...
\hfill \\
\hfill \\
El fichero de configuración, en nuestro caso en la ruta \textit{/etc/apache2/sites-available/sitednie.conf} para Apache es el siguiente:
...
A continuación hay que editar el fichero \textit{/etc/apache2/ports.conf} si es necesario:
\begin{lstlisting}[language=bash]
