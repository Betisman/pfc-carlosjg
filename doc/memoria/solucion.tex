...
\epigraphhead[70]{\epigraph{Do not mistake that the ballot is stronger than the bullet.}{\textit{Abraham Lincoln\footnotemark[\value{epihc}]}}}
\footnotetext[\value{epihc}]{{16º Presidente de los Estados Unidos de América (1861-1865) \url{https://es.wikipedia.org/wiki/Abraham_Lincoln} Fuente: \url{http://www.bartleby.com/73/1903.html}}}
...
% *************************************************************************************************************** %
	...
	%\end{figure}
		...
		Este prototipo trata de demostrar que se puede implementar un sistema de voto seguro que utilice el \gls{DNIe} 3.0 como herramienta de identificación digital del votante. De hecho, es la primera implementación que utiliza esta tecnología para un proyecto electoral.
		...
		\section{Arquitectura del sistema}\label{solucion_arquitectura}
			...
			\begin{description}
				...
				\item[Sistema de autenticación:] Para permitir el acceso al sistema con las credenciales del \gls{DNIe}, se utiliza un sistema de autenticación basado en el protocolo oAuth2. El usuario accede a través de un navegador web o de la app Android desarrollada por \gls{HTTPS} de forma segura al servidor web. Se establece un canal seguro utilizando el certificado del propio servidor y como certificado de cliente, el de identificación del \gls{DNIe}. Este sistema es el que autoriza al usuario comprobando su identidad y consultando su presencia en el censo de la elección.
			
				\item[Sistema de votación:] El sistema de votación es una versión adaptada del sistema Helios Voting. Se comunica con el cliente (y con el sistema de autenticación) por \gls{HTTPS} y el usuario, autorizado previamente, tiene la posibilidad de, dependiendo de su rol, crear y gestionar una elección, votar en ella y/o, simplemente, consultar los resultados.
			\end{description}
			
			\begin{figure}[!ht]
				\centering
					\includegraphics[width=.95\textwidth]{imgs/arquitecturaSistema00.png}
		...
		\section{Sistema de autenticación}\label{arquitectura_servidorAutenticacion}
			...
			El servidor web se configura para exponer un endpoint de entrada seguro, permitiendo el acceso por \gls{HTTPS}. En dicha configuración es muy importante activar diversas características:
			
			\subsection{Servidor web:}
				El canal de comunicación entre el usuario y el sistema de autenticación es la web, el protocolo es \gls{HTTPS} y la herramienta que gestiona estos accesos es el servidor web.
				
				Con el fin de establecer un canal \gls{HTTPS} seguro entre usuario y servidor, es necesario que en su configuración se contemplen tres certificados, dos de ellos en posesión del servidor y el tercero de obligada presentación por parte del cliente.
				...
				\begin{description}
					...
					En nuestro caso, se ha decidido utilizar un certificado autofirmado para el prototipo implementado. Los pasos necesarios tanto para crear y firmar el certificado como para configurar el servidor web del Sistema de Votación (anexo \ref{App:ConfiguracionServidorWeb}).
					
					\item[Certificado AC Raíz DGP:] Es el certificado que permite al Sistema Operativo y al navegador web reconocer la Autoridad de Certificación de los certificados de sede electrónica, la AC Raíz DGP, que son los encargados de generar los certificados que se incluyen en el \gls{DNIe} 3.0. Se utiliza en la app Android y en el servidor web del Sistema de Autenticación (anexo \ref{App:ConfiguracionServidorWebOauth}).
					
					\item[Certificado de cliente:] Para utilizar el \gls{DNIe} como fuente de identificación del usuario, es preciso configurar el servidor web para que requiera en la conexión la presencia del certificado de autenticación del \gls{DNIe} del usuario. Para ello, el servidor debe poseer el certificado raíz que provee la Dirección General de la Policía. Este certificado se configura en el servidor. Se puede consultar el anexo \ref{App:ConfiguracionServidorWebOauth} para ver cómo realizarlo.
...
class AuthorizeView(View):
  ...
  def get(self, request, *args, **kwargs):
      ...
      try:
        ...
        \gls{DNIe} = get_dni_info_from_ssl(request)
      except Exception as e:
        logger.warn('get_No vienen las credenciales del \gls{DNIe}')

      return self._render(request=request, form=form, \gls{DNIe}=dnie)
\end{lstlisting}
...
\begin{lstlisting}
  ...
  2017-01-09 18:23:10,193 DEBUG SSL_CLIENT_I_DN: CN=AC \gls{DNIe} 001,OU=DNIE,O=DIRECCION GENERAL DE LA POLICIA,C=ES
...
\end{lstlisting}
				...
				Por su parte, aquellas que comienzan por SSL\_CLIENT son las que contienen la información proporcionada por el certificado presentado por el cliente. En el caso de este proyecto, contendrán los datos proporcionados por el certificado de autenticación del \gls{DNIe}.
...
\end{lstlisting}
						...
						\begin{description}
							...
							\item [serialNumber:] 11062005B $\Leftarrow$ En el caso del \gls{DNIe}, en serialNumber encontraremos el número del DNI de la persona identificada por el documento.
...
\end{lstlisting}
			...
			\subsection{Aplicación de autenticación}\label{solucion:aplicacion:autenticacion}
			...
			\subsection{OCSP}
				...
				\gls{OCSP} es un servicio de revocación que ofrece la autoridad que crea el certificado raíz, en nuestro caso la \gls{DGP} y la \gls{FNMT}. Estas organizaciones proveen de un servicio al cual conectarse y consultar la validez de los certificados que se estén tratando de autenticar.
			...
			\section{Sistema de votación}\label{arquitectura_servidorVotacion}
				...
				\begin{figure}[!ht]
					\centering
						\includegraphics[width=.99\textwidth]{imgs/arquitecturaSistema.png}
			...
			\subsection{Estructura}
				...
				\subsubsection*{Core}
					...
					Estas son las clases que se definen en este módulo (figura \ref{fig:diagrama.clases.01}):
					\begin{description}[font=$\cdot$\scshape\bfseries]
					%labelindent=\prindent\parindent]
						\item [Election]
					...
					\begin{figure}[H]
						\centering
					  \includegraphics{imgs/diagramaClases01}
					  \caption{Clases del módulo central de Helios.}
					  \label{fig:diagrama.clases.01}
					\end{figure}
				...
				\subsubsection*{Autenticación}
					...
					En el caso del login por \gls{DNIe}, este subsistema es el que maneja la comunicación con el Sistema de Autenticación (\ref{arquitectura_servidorAutenticacion}).
					...
					\begin{figure}[H]
						\centering
					  \includegraphics{imgs/folders_auth_system}
					  \caption{Carpeta auth\_system que contiene los módulos de autenticación del sistema.}
					  \label{fig:folders.auth.system}
					\end{figure}

					
					Por defecto, Helios ha implementado ya los siguientes módulos para permitir la identificación federada:
					\begin{itemize}
						\item Facebook $\Leftarrow$ helios\_auth/auth\_system/facebook.py
						\item CAS $\Leftarrow$ helios\_auth/auth\_system/cas.py
						\item Clever.com $\Leftarrow$ helios\_auth/auth\_system/clever.py
						\item Google $\Leftarrow$ helios\_auth/auth\_system/google.py
						\item LinkedIn $\Leftarrow$ helios\_auth/auth\_system/linkedin.py
						\item Twitter $\Leftarrow$ helios\_auth/auth\_system/twitter.py
						\item Yahoo!	 $\Leftarrow$ helios\_auth/auth\_system/yahoo.py
					\end{itemize}
					...
					\begin{itemize}
						\item User/password $\Leftarrow$ helios\_auth/auth\_system/password.py
					\end{itemize}
					...
					\begin{itemize}
						\item \gls{DNIe} $\Leftarrow$ helios\_auth/auth\_system/dnie.py
					\end{itemize}
					
					Define una clase \textbf{\gls{DNIe}} con una serie de miembros que almacenan la información del usuario recibida desde el Sistema de Autenticación, del certificado digital de autenticación del \gls{DNIe}.
  ...
  def do_logout(request)
					...
					Se puede observar en la definición de estas funciones que son las que llaman al Sistema de Autenticación oAuth o acaban siendo llamadas por los callbacks que a aquél se pasan en el flujo de comunicación que implementa este protocolo.
				...
				\subsubsection*{Cabina de votación}
				...
				\subsubsection*{Verificador de voto}
		...
		\section{Cliente Android}\label{arquitectura_cliente}
			...
			Esto no es posible, sin embargo, en dispositivos Android. No existen drivers liberados para acceder a los certificados del \gls{DNIe} 3.0 por NFC desde el Sistema Operativo Android y comunicarlo con el navegador web, al contrario de los que existen para Windows, Linux o MacOS. No es posible por ahora, por tanto que un navegador web en Android (ni iOS) tenga acceso a los certificados como ocurre en un navegador de un equipo de escritorio. 
			...
			\begin{figure}[!ht]
				\centering
			 	\includegraphics[width=0.90\textwidth]{imgs/arquitecturaAndroidApp.png}
			  	\caption{Arquitectura lógica de la App sobre \gls{DNIe}Droid\cite{dnieAutenticacion:1.0}}
			  	\label{fig:arquitecturaAppDnieDroid}
			\end{figure}
		
			\subsection{Código fuente}\label{solucion_app_codigofuente}
				El código fuente de la app original de la \gls{DGP} ha debido ser modificado ya que la funcionalidad que se busca que tenga la aplicación no es la misma que la que ofrece la original.
				...
				Una vez se abra la app, ya sea desde el menú del dispositivo o desde el botón de login de la página inicial de la web del sistema de votación, aparecerá el menú de selección del \gls{CAN} del \gls{DNIe} que queremos utilizar, o, si es la primera vez que cierto usuario utiliza la app, la posibilidad de añadir un nuevo \gls{DNIe} con su \gls{CAN} correspondiente.
			...
			\subsection{App Android de Autenticación}\label{solucion_appAndroidAutenticacion}
					...
					La app original mostraba un ejemplo de funcionamiento en el que se accedía a varias webs que necesitaban acceso con identificación por \gls{DNIe}. Las modificaciones que se han realizado a esta app han consistido en:
					\begin{itemize}
						...
						\item Para ello, se ha modificado el software cambiando el flujo de llamadas. Originalmente sólo se hacía una llamada a una web para obtener información. Para cumplir con el protocolo oAuth, se ha incrementado el número de llamadas \gls{HTTPS} de la app, con una mayor comunicación bidireccional entre el servicio y el dispositivo.
						...
						\item En la app original, se realiza una llamada \gls{HTTPS}, se obtiene la información y se muestra en una página estática dentro de un WebView. Los WebViews en Android no soportan navegación manteniendo certificado de cliente, por lo que no es útil para la nueva funcionalidad de nuestra app. Por ello, se ha eliminado esta forma de mostrar información. Ahora se realizan las comunicaciones del flujo oAuth y cuando el usuario ha sido identificado y autenticado, se redirige a la web del sistema de voto por medio de un navegador externo a la app.
					\end{itemize}
...
\end{lstlisting}
					...
					\begin{figure}[H]
						...
						\caption{App Autenticación Android: Esperando que se aproxime el \gls{DNIe} 3.0.}
					...
					Para poder llegar a utilizar los certificados, no obstante, primero se solicitará el \gls{PIN} al usuario (figura \ref{fig:login.android.DNIe.PIN}), con el cual se autentica al mismo contra el \gls{DNIe} y se le conceden permiso de acceso a las claves. A partir de este momento, la comunicación entre documento y dispositivo va cifrada y se pueden acceder a los certificados.
					...
					\begin{figure}[H]
							...
							\includegraphics[height=6cm]{imgs/login_Android/android_090.png}
							...
							\includegraphics[height=6cm]{imgs/login_Android/android_100.png}
				
							\caption{App Autenticación Android: Recepción de código oAuth.}
				
							\label{fig:login.android.recepcionCodigooAuth}    	\end{minipage}
					\end{figure}
...
\normalsize					
					...
					Una vez asumimos el certificado autofirmado, por fin accedemos a la web del sistema de voto logados con la información contenida en el certificado de autenticación de nuestro \gls{DNIe} 3.0 (figura \ref{fig:login.android.logado}).
					...
					\begin{figure}[H]
						...
						\end{subfigure}
						\caption{App Autenticación Android: Acceso al sistema de voto con login con el \gls{DNIe}.}
		...
		\section{Topología de red}\label{solucion_topologiaRed}
				...
				\begin{enumerate}
					\item \textbf{Servidor de autenticación y Servidor de votación en la misma red. Acceso de los votantes desde Internet.} Figura \ref{fig:topologiaRed_servidoresMismaRed}
						...
						Las consultas \gls{OCSP} para validación del \gls{DNIe} se realizan a través de Internet.
						\newline
						
						
						\begin{figure}[H]
							\centering
								\includegraphics[width=0.80\textwidth]{imgs/topologiaRed01.png}
					...
					\item \textbf{Servidor de autenticación y Servidor de Votación en redes diferentes. Acceso de los votantes desde Internet.} Figura \ref{fig:topologiaRed_servidoresDistintaRed}
						...
						Esta topología no requiere port forwarding, pues cada servidor tiene su propio acceso a/por Internet. Los dispositivos accederían directamente a la IP/\gls{URL} de cada servidor y utilizarían sus diferentes servicios utilizando el puerto correspondiente.
						...
						\begin{figure}[H]
							\centering
								\includegraphics[width=0.80\textwidth]{imgs/topologiaRed02.png}
					...
					\item \textbf{Tanto el Servidor de autenticación, como el Servidor de Votación y los dispositivos se comunican por la misma red, sin necesitar acceder a través de Internet.} Figura \ref{fig:topologiaRed_intranet}
						...
						Haría falta un acceso a Internet para poder realizar consultas OCSP de validación del \gls{DNIe}.
						...
						\begin{figure}[H]
							\centering
								\includegraphics[width=0.80\textwidth]{imgs/topologiaRed03.png}
		...
		\end{description}
			...
			El proyecto original no se adecua a las necesidades del sistema que se quiere integrar, pues no soporta certificados digitales. Esta es, pues, una de las funcionalidades que hay que agregar para poder integrar ambos sistemas.
			...
			\begin{description}[font=$\bullet$\scshape\bfseries]
				...
				\item[mod\_ssl]: Módulo que permite a ciertos servidores web utilizar certificados digitales como forma de autenticación. En el caso de nuestro proyecto, servirán tanto para establecer los canales \gls{SSL}/\gls{TLS} como para incluir el certificado digital de autenticación del \gls{DNIe} en el canal seguro, recuperar la información de usuario del certificado y pasarla al código del servidor.
			\end{description}
			
			
			Es imprescindible que el servidor web esté configurado para aceptar tan sólo comunicaciones seguras.
			
			Otra configuración básica del servidor será que requiera que las comunicaciones con el votante, para su autenticación, deban realizarse presentando éste los certificados de su \gls{DNIe}.
			
			Se puede ver la forma en que ambas configuraciones se han realizado en el prototipo (\ref{solucion_prototipo}) en el anexo \ref{App:ConfiguracionServidorWeb}.

				\hfill \\
			
			\textbf{Flujo de comunicación oAuth}
			
			El flujo del proceso de identificación del usuario a través del protocolo oAuth2 modificado se puede observar en la figura \ref{fig:flujo.oauth}, donde se esquematizan los siguientes pasos:
			
					\begin{figure}[!htbp]
						\centering
							\includegraphics[width=.95\textwidth]{imgs/flujoOAuth20.png}
						\caption{Flujo oAuth para servicio de autenticación}
						\label{fig:flujo.oauth}
					\end{figure}
										
					\begin{enumerate}
						\item La app del \gls{DNIe} obtiene las credenciales del chip de identificación del documento por \gls{NFC}.
						...
						\item La app del \gls{DNIe} lanza un \textit{intent} de Android para abrir el navegador web por defecto del dispositivo en la página de inicio del Sistema de Votación, pasándole además el \textit{access code}. Aquí el entorno del dispositivo termina en la app y pasa al navegador web.
						\item Desde el navegador, se lanza una petición de acceso al Sistema de Votación con el \textit{access code} y una \gls{URL} de \textit{callback} como parámetros.
						\item El Sistema de Votación realiza una petición a la \gls{URL} del Sistema de Autenticación que va en el \textit{callback} (/api/token) con el \textit{access code} como parámetro.
				...
				\subsection{Votación}\label{esquemaVotacion_votacion}
					...
					El módulo de votación es un fichero HTML, vote.html, orquestado por código Javascript, escrito dentro del mismo fichero entre etiquetas <script></script> (ver figura \ref{fig:esquema.cabina.votacion}).
					
					\begin{figure}[!ht]
						\centering
							\includegraphics[width=.95\textwidth]{imgs/cabinaVotacion01.png}
						\caption{Esquema de la Cabina de Votación}
						\label{fig:esquema.cabina.votacion}
					\end{figure}
					...
					Durante todo el proceso de votación, aunque éste se divide en tres pasos, el votante se encuentra en la misma página \textit{vote.html}. La orquestación entre una fase y la siguiente se gestiona a través de código Javascript.
					
					\begin{figure}[H]
						\centering
							\includegraphics[width=.95\textwidth]{imgs/diagramaEstadosVotacion01.png}
					...
					\begin{description}
						\item[Inicio]\hfill \
							...
							\begin{figure}[H]
								\centering
								\includegraphics[height=6cm]{imgs/screenshots/20.png}
								\caption{Carga de la cabina de votación.}
								\label{fig:smartphone.cabina.votacion}
							\end{figure}
							
							\begin{figure}[H]
					    		\begin{minipage}{0.45\textwidth}
					        		\centering
									\includegraphics[height=6cm]{imgs/screenshots/22.png}
					    		\end{minipage}\hfill
					    		\begin{minipage}{0.45\textwidth}
						        	\centering
									\includegraphics[height=6cm]{imgs/screenshots/24.png}
								\end{minipage}								\caption{Inicio de la cabina de votación.}
								\label{fig:smartphone.inicio.cabina.votacion}
							\end{figure}
						...
						\item[Paso 1. Selección de candidatos] \hfill \
							...
							Ver figura \ref{fig:smartphone.eleccion.candidatos}.
	
							\begin{figure}[H]
					    		\begin{minipage}{0.45\textwidth}
					        		\centering
									\includegraphics[height=6cm]{imgs/screenshots/30.png}
					    		\end{minipage}\hfill
					    		\begin{minipage}{0.45\textwidth}
						        	\centering
									\includegraphics[height=6cm]{imgs/screenshots/32.png}
								\end{minipage}								\caption{Elección de candidatos}
								\label{fig:smartphone.eleccion.candidatos}
							\end{figure}
...
\normalsize
							...
							\begin{figure}[H]
								\centering
								\includegraphics[height=6cm]{imgs/screenshots/40.png}
								\caption{Pantalla de espera durante el proceso de cifrado del voto.}
								\label{fig:smartphone.cifrado.voto}
							\end{figure}
						...
						\item[Paso 2. Revisar el voto] \hfill \
							...
							\begin{figure}[H]
					    		\begin{minipage}{0.32\textwidth}
					        		\centering
									\includegraphics[height=6cm]{imgs/screenshots/50.png}
					    		\end{minipage}\hfill
					    		\begin{minipage}{0.32\textwidth}
						        	\centering
									\includegraphics[height=6cm]{imgs/screenshots/52.png}
								\end{minipage}
								\begin{minipage}{0.32\textwidth}
						        	\centering
									\includegraphics[height=6cm]{imgs/screenshots/54.png}
								\end{minipage}								\caption{Pantalla de revisión del voto.}
								\label{fig:smartphone.revision.voto}
							\end{figure}
						...
						\item[Paso 3. Emitir el voto] \hfill \
							...
							\begin{figure}[H]
					    		\begin{minipage}{0.45\textwidth}
					        		\centering
									\includegraphics[height=6cm]{imgs/screenshots/70.png}
									\caption{Pantalla de emisión del voto.}
									\label{fig:smartphone.emision.voto}
					    		\end{minipage}\hfill
					    		\begin{minipage}{0.45\textwidth}
						        	\centering
									\includegraphics[height=6cm]{imgs/screenshots/22.png}
									\caption{Pantalla de confirmación de voto emitido.}
									\label{fig:smartphone.confirmacion.voto}
								\end{minipage}
							\end{figure}
						...
						\item[Auditar el voto] \hfill \
							...
							\begin{figure}[H]
					    		\begin{minipage}{0.24\textwidth}
					        		\centering
									\includegraphics[height=6cm]{imgs/screenshots/60.png}
					    		\end{minipage}
					    		\begin{minipage}{0.24\textwidth}
						        	\centering
									\includegraphics[height=6cm]{imgs/screenshots/62.png}
								\end{minipage}
								\begin{minipage}{0.24\textwidth}
					        		\centering
									\includegraphics[height=6cm]{imgs/screenshots/64.png}
					    		\end{minipage}
					    		\begin{minipage}{0.24\textwidth}
						        	\centering
									\includegraphics[height=6cm]{imgs/screenshots/69.png}
								\end{minipage}								\caption{Pantalla con información para auditar el voto y publicarlo.}
								\label{fig:smartphone.auditoria.voto}
							\end{figure}
						...
						\item[Verificación del voto] \hfill \
							...
							Requiere como entrada de datos el JSON del voto y la \gls{URL} de la elección.
...
\end{lstlisting}
					    		...
					    		\begin{minipage}{0.45\textwidth}
					        		\centering
									\includegraphics[height=6cm]{imgs/screenshots/66.png}
					    		\end{minipage}\hfill
					    		\begin{minipage}{0.45\textwidth}
						        	\centering
									\includegraphics[height=6cm]{imgs/screenshots/68.png}
								\end{minipage}								\caption{Pantalla del verificador de voto individual.}
								\label{fig:smartphone.verificar.voto}
							\end{figure}
					\end{description} 
				...
				\subsection{Totalización}\label{solucion_totalizacion}
					...
					Por medio del homomorfismo (\ref{primitivaCifradoHomomorfico}), no es necesario que se descifren los votos para totalizar el resultado de la elección.
					
					Los votos no se descifran, se totalizan los votos cifrados y lo que se descifra es el resultado de esta suma. Este proceso se realiza basándose en la propiedad de cifrado homomórfico aditivo que ofrece el algoritmo de cifrado de ElGamal Exponencial (\ref{primitivaCifradoHomomorfico_elgamal_exponencial}).
 ...
 def add_vote(self, encrypted_vote, verify_p=True):
    ...
    self.num_tallied += 1
		...
		Del código anterior se puede obtener que, efectivamente, durante el proceso de totalización (\textit{def compute\_tally()}), el sistema se dedica a sumar resultados (\textit{tally.add\_vote(..)}) que están cifrados y en ningún momento se dispone a descifrarlos para realizar la suma.
		\hfill \\
...
\end{lstlisting}
	...
	Sin embargo, cuando hay más trustees, una vez el sistema descifra su parte, avisa al resto de trustees de que deben descifrar su parte con su clave y queda a la espera de que esto ocurra.
				...
				\begin{enumerate}
					\item Se presentan los resultados a los administradores. Sólo estos tienen permiso para ver los datos.
					\item Una vez los administradores lo crean convenientes, son ellos los que publican los resultados para que puedan ser consultados públicamente.
				\end{enumerate}
				...
				\begin{figure}[H]
					\centering
					\includegraphics[height=6cm]{imgs/screenshots/80.png}
					\caption{Tabla de resultados del escrutinio de una elección simple.}
					\label{fig:smartphone.escrutinio}
				\end{figure}
				
				Con respecto al modelo original de Helios, en la difusión se han realizado varias mejoras:
				
				\begin{itemize}
					\item Se ha actualizado la interfaz gráfica de la página de resultados, cumpliendo en lo posible con el estándar \gls{W3C} y tratando de dar visibilidad a los resultados.
					\item Se han añadido datos electorales como el del número de votantes y la abstención. Esta información era fundamental que se añadiese y en los requisitos del sistema así se estipulaba. Además de informar al observador, dota de transparencia al sistema.
					\item Se han añadido funcionalidades tales como poder ordenar la tabla de resultados según la columna seleccionada.
					\item Se ha integrado una suite de exportación por la cual los resultados se pueden exportar a diferentes formatos, como PDF, CSV ó Excel. Así se ofrece la oportunidad a los observadores que puedan obtener realizar los análisis que crean oportunos sobre los datos.
					\item Incluso es posible compartir los resultados por redes sociales.
				\end{itemize}
			...
			\section{Diseño de la interfaz de usuario}\label{diseño_interfaz_usuario}
				...
				\subsection{Estructura de la aplicación móvil}
					...
					La interfaz de usuario de la app Android necesaria para ingresar en el sistema con los certificados digitales del \gls{DNIe} 3.0 no ha sido modificada ostensiblemente.
					
					Aunque es una parte primordial del sistema y su adaptación ha requerido bastante estudio y desarrollo, realmente al final no se utiliza durante un tiempo suficiente. Por ello, se ha reutilizado la diseñada originariamente por los desarrolladores de la app original de la \gls{DGP}, con las modificaciones necesarias debidas al cambio de funcionalidad de la aplicación.
				...
				\subsection{Accesibilidad}
					...
					Es importante que la web sea accesible, que usuarios con diversas discapacidades tengan la posibilidad de utilizarla de forma independiente.
					
					Detalles que se han tenido en cuenta para procurar la mayor accesibilidad del sistema han sido:
					
					\begin{description}
						\item [\gls{WCAG} 2.0] El diseño de la web ha sido modificado para cumplir con el máximo de pautas posibles marcadas en el estándar de accesibilidad \gls{WCAG} 2.0\footnote{\url{http://www.sidar.org/traducciones/wcag20/es/}} propuesto por el consorcio \gls{W3C}.
						\item [Colores] Los colores de la web han sido elegidos para que el contraste cumpla con lo estipulado en el estándar.
						\item [Audio] El código \gls{HTML} de la web ha sido modificado añadiendo las etiquetas e información necesarias para que se puedan utilizar sistemas de lectura de pantalla para personas con una importante capacidad visual.
					\end{description}
%\iffalse					
...
%				
%				Así, los documentos válidos para ejercer el derecho al voto serán el \gls{DNIe} (tanto la primera versión como la denominada 3.0, presentada en enero de 2015) y la TUI \notasMejorar{***InfoTUI***} de la Universidad San Pablo-CEU. En sendos documentos encontramos elementos criptográficos que identifican unívocamente a su dueño. Además encontramos en ellos certificados para la firma digital, que serán necesarios para la fase de votación.
%				
%				El votante se identifica con su documento digital de forma remota. Es necesario que disponga de un lector de chip electrónico conectado al dispositivo desde el que va a realizar el voto, aunque utilizando \gls{DNIe} con lector de chip sin contacto, no haría falta si se hace uso de un dispositivo con sensor de radiofrecuencia, con capacidad para leer información a través de NFC.
%				
%				A través de la app Android (o la app web), el votante accede al servicio de votación por Internet. El primer paso es la identificación del votante. Es la primera vez que hará uso de los certificados del \gls{DNIe}. En este caso, la app leerá (con NFC o chip con contacto) el certificado de Autenticación del \gls{DNIe}, por el cual se asegura la identidad del votante. Con la identidad del votante verificada (por la DGP), se contrasta con el censo, para comprobar:
%				\begin{itemize}
...
%\fi		
	...
	\section{Prototipo}\label{solucion_prototipo}
		...
		\begin{itemize}
			...
			\item App Android de identificación con \gls{DNIe}
		\end{itemize}
		...
		\subsection{Arquitectura física}\label{solucion_prototipo_arquitecturaFisica}
			...
			\begin{description}
				...
				\item[Servidor de votación]	Raspberry Pi Model 3 B (fig. \ref{fig:prototipo.raspberry.pi.3B.servidor.votacion})
		
				\item[Servidor de identificación y autenticación] Raspberry Pi Model B (Rev. 2.0, 512Mb) (fig. \ref{fig:prototipo.raspberry.pi.B.servidor.autenticacion})
			...
			\begin{figure}[H]
		    		...
		    		\begin{minipage}{0.45\textwidth}
		        		\centering
					\includegraphics[width=0.80\textwidth]{imgs/prototipoRaspberryPi01.png}
					\caption{Raspberry Pi Model 3 B, utilizada para albergar el Servidor de votación}
					\label{fig:prototipo.raspberry.pi.3B.servidor.votacion}
		    		\end{minipage}\hfill
		    		\begin{minipage}{0.45\textwidth}
			        	\centering
					\includegraphics[width=0.90\textwidth]{imgs/prototipoRaspberryPi02.png}
					\caption{Raspberry Pi Model B (Rev. 2.0, 512Mb), utilizada para albergar el Servidor de identificación y autenticación}
					\label{fig:prototipo.raspberry.pi.B.servidor.autenticacion}
				\end{minipage}
			\end{figure}
		...
		\begin{table}[ht]
			...
			\setstretch{1}
				\begin{threeparttable}
					...
					\multirow{4}{*}{\textbf{Autenticación}} 	& 8662 & 192.168.1.144 & 442 	& Puerto de acceso al servidor Apache por \gls{HTTPS} \tabularnewline
						
					\cline{2-5}
						
					& 8888 & 192.168.1.144 & 80 	& Puerto de acceso al servidor Apache por \gls{HTTP} \tabularnewline
						
					\cline{2-5}
						
					& 8443 & 192.168.1.144 & 443 	& Puerto de acceso al servidor Apache por \gls{HTTPS} \tabularnewline
					...
					\multirow{3}{*}{\textbf{Votación}}	 			& 8889 & 192.168.1.145 & 80 	& Puerto de acceso al servidor Apache por \gls{HTTP} \tabularnewline
						
					\cline{2-5}
						
					& 8445 & 192.168.1.145 & 443 	& Puerto de acceso al servidor Apache por \gls{HTTPS} \tabularnewline
		...
		El router utilizado para el prototipo ligero ha sido un TP-LINK TL-WR702N\footnote{\url{http://www.tp-link.com/ar/products/details/TL-WR702N.html}} (fig. \ref{fig:prototipo.router.tplink}). Se trata de un modelo de router muy simple y ultraportable, con lo que se manifiesta la facilidad de puesta en producción de una elección a pequeña escala que resulte asequible económicamente.
		
		\begin{figure}[H]
			\centering
				\includegraphics[width=.45\textwidth]{imgs/prototipoRouterTPLINK.png}
			\caption{Router TP-LINK TL-WR702N utilizado para la construcción del prototipo.}
			\label{fig:prototipo.router.tplink}
		\end{figure}
