	...
	%\end{figure}
		...
		\end{description}
			...
			\end{description}
					...
					\subsubsection{App Android de Autenticación}
					...
					De hecho, las modificaciones han consistido en eliminar funcionalidades y adaptar una de ellas. Básicamente, en lugar de hacer una llamada a una web, como realizaba la original, ha sido preciso modificar el flujo para que realice varias para cumplir con una autenticación con un protocolo oAuth.
					
					En cada una de estas llamadas, la aplicación incluye en la comunicación HTTPS (TLS) los certificados clientes del DNIe 3.0.
					...
					En este momento, cualquiera que sea el camino que se ha seguido, el usuario entra en la app. Con respecto a la app original, se ha eliminado el menú principal y todas las funcionalidades que aportaba. Directamente, se muestra la pantalla de introducción o selección del \gls{CAN}. El \gls{CAN} es un identificador de 6 dígitos que aparece en el anverso del documento físico del \gls{DNIe}. Se utiliza para establecer un canal cifrado \gls{PACE} entre el documento y el dispositivo.
					...
					En caso de intentar una lectura de la información de un \gls{DNIe} introduciendo un \gls{CAN} incorrecto se produciría un error al establecer el canal \gls{PACE} y no sería posible la comunicación entre documento y dispositivo.
					
					
					
					Cuando se ha indicado el \gls{CAN} correcto del documento que se va a utilizar para logarse en el sistema, la aplicación solicita al usuario que acerque su documento físico al dispositivo para que este pueda acceder a los certificados.
					
					La comunicación entre documento y dispositivo se realiza mediante \gls{NFC} (\ref{estadoNFC}). Según el documento \cite{dnieAutenticacion:1.0}, los desarrolladores de la app de ejemplo indican que \textit{''por motivos de hardware y de seguridad, distancia máxima soportada por NFC es, dependiendo del dispositivo, de alrededor de un centímetro''}. De todos modos, es una comunicación ciertamente sensible, que se ve afectada por la distancia y el movimiento, necesitando para un correcto funcionamiento cercanía y una notable estabilidad.
					
					
					Una vez estén cerca documento y dispositivo, se detectarán y establecerán la comunicación cifrada entre ambos, a través de la cual podrán intercambiarse tanto comandos como respuestas a estos, accediendo a la información alojada en los certificados y permitiendo a estos firmar información presentada por el dispositivo (aunque en nuestra app no hará falta utilizar el certificado de firma digital).
					
					
					
					Para poder llegar a utilizar los certificados, no obstante, primero se solicitará el \gls{PIN} al usuario, con el cual se autentica al mismo contra el DNIe y se le conceden permiso de acceso a las claves. A partir de este momento, la comunicación entre documento y dispositivo va cifrada y se pueden acceder a los certificados.
					...
					Esta keystore con los dos cetrtificados se debe incluir en el proyecto Android con un formato BKS, lo cual se consigue realizando los pasos expuestos en el anexo \ref{App:exportarCertsAppAndroid}. Una vez obtenido el fichero keystore que contiene los dos ceritficados, ha de colocarse en la carpeta del proyecto:
					
					\small\begin{verbatim}
					clienteAndroidDNIeoAuth2/app/src/main/res/raw
					\end{verbatim}
					
					Habrá que editar también el fichero
					
					\small\begin{verbatim}
					clienteAndroidDNIeoAuth2/app/src/main/res/values/trusted_hosts.xml
					\end{verbatim}
					
					para añadir el hash del certificado del servidor. Este hash se obtiene en el servidor con el comando que se indica igualmente en el anexo \ref{App:exportarCertsAppAndroid}. Así, el fichero que indica qué servidores son confiables y que utiliza la app para poder establecer una conexión segura con el servidor quedaría parecido a este:+
...
<resources xmlns:android="http://schemas.android.com/apk/res/android">
  <array name="trusted_hosts">
    ...
    <!-- En el servidor: openssl x509 -fingerprint -noout -in server.crt -->
					...
					El servidor de autenticación establece un canal seguro con su certificado (en el prototipo será autofirmado) pero valida el certificado de autenticación con el certificado AC Raíz de la Policía que posee y que se ha configurado para ser utilizado.
					...
					\begin{figure}[!ht]
						...
						\caption{App Autenticación Android: Introducir PIN del DNIe 3.0}
					...
					\begin{figure}[!ht]
						...
						\caption{App Autenticación Android: PIN del DNIe 3.0 introducido}
