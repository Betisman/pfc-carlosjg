			...
			\subsection{Primitivas criptográficas}\label{primitivasCriptografia}
				...
				Antes de entrar en los diferentes esquemas de voto electrónico (\ref{esquemasVotoElectronico}), introducimos una serie de primitivas criptográficas que se utilizan en ellos.				
					...
					\subsubsection{Cifrado homomórfico}\label{primitivaCifradoHomomorfico}
					    ...
					    Hay algoritmos en los que las funciones que se aplican ($f$ y $g$) son idénticas, de forma que ($f$ = $g$), lo cual resulta muy interesante ya que aplicando la misma función a un mensaje cifrado, al descifrar se obtiene el mismo resultado que se resultaría de aplicar la función al mensaje en claro.
					    
					    $\delta(f(\varepsilon(m, k)), k) = f(m)$
						
						En la figura \ref{fig:homomorfismo} se muestra un ejemplo de cifrado homomórfico basado en la aplicación de la operación multiplicación.
