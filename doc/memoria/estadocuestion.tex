			...
			\subsection{Primitivas criptográficas}\label{primitivasCriptografia}
				...
				Antes de entrar en los diferentes esquemas de voto electrónico (\ref{esquemasVotoElectronico}), introducimos una serie de primitivas criptográficas que se utilizan en ellos.				
					...
					\subsubsection{Pruebas de conocimiento nulo}\label{primitivaPruebasConocimientoNulo}
						...
						Un sencillo ejemplo para entender el concepto de este tipo de protocolos lo encontramos en una publicación de Pablo Della Paolera, astrónomo de la Universidad Nacional de La Plata (Argentina), en su blog personal \cite{dellaPaolera2014}. En esta publicación explica que se puede demostrar \textit{algo} correctamente sin necesidad de demostrar \textit{por qué}.
			\begin{center}
				\small
				\fbox{
					\begin{minipage}{0.9\textwidth}
						Basándonos en el ejemplo publicado, supongamos dos actores $A$ y $B$. 
						
						$B$ quiere demostrar que \textit{$A$ tiene (o no) la misma cantidad de monedas en su bolsillo izquierdo que en el derecho}. 
						
						Para ello, la forma más simple de hacerlo sería que $B$ le pidiese a $A$ las monedas de cada bolsillo y las contase. Por tanto, para demostrar la afirmación de que \textit{$A$ realmente tiene (o no) las mismas monedas en cada bolsillo} basta con que las enseñe.
